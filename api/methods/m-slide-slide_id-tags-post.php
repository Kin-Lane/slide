...
$app->post($route, function ($Slide_ID)  use ($app){
	...
	if(isset($param['tag']))
		...
		$CheckTagPivotQuery = "SELECT * FROM slide_tag_pivot where Tag_ID = " . trim($Tag_ID) . " AND Slide_ID = " . trim($Slide_ID);
		...
		if($CheckTagPivotResult && mysql_num_rows($CheckTagPivotResult))
			...
			{
			$query = "INSERT INTO slide_tag_pivot(Tag_ID,Slide_ID) VALUES(" . $Tag_ID . "," . $Slide_ID . "); ";
			mysql_query($query) or die('Query failed: ' . mysql_error());
			}
